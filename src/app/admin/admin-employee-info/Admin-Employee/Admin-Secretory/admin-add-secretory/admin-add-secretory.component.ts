...
@Component({
  selector: 'app-admin-add-secretory',
  templateUrl: './admin-add-secretory.component.html',
  styleUrls: ['./admin-add-secretory.component.css']
})
export class AdminAddSecretoryComponent implements OnInit {
  ...
  getEmail(){
    ...
    this.adminService.getSecretoryEmail()
    .subscribe(
      data => {
        this.email = data;
        // console.log(data);
  ...
  onSave(){
    ...
    if(age >= 18 && !check){
      ...
      if(this.formData.valid){
        this.adminService.addSecretory(this.formData.value , this.setProperDate(this.formData.get('dob').value))
        .subscribe(data => {
          alert(data);
